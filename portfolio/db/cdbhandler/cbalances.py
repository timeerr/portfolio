...
def add_account(account: str, token: str, amount: float,
                _type: str, kyc: str, description: str):
    token = token.lower()
...
def get_total_token_balance(token: str):
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(f"SELECT amount FROM cbalances WHERE token='{token}'")
...
def get_all_accounts_with_amount():
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT account,token,amount FROM cbalances")
        result = cursor.fetchall()

        # Convert to BTC
        accounts = {d[0]: 0 for d in result}
        for r in result:
            acc, token, amount = r  # Unpack
...
def update_balance(account: str, token: str, newbalance: float):
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"UPDATE cbalances SET amount = {newbalance} \
...
def update_kyc(account: str, token: str, newkyc: str):
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"UPDATE cbalances SET kyc = '{newkyc}' \
            WHERE account = '{account}' AND token = '{token}' ")
        conn.commit()
...
def update_description(account: str, token: str, newdescription: str):
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"UPDATE cbalances SET description = '{newdescription}' \
            WHERE account = '{account}' AND token = '{token}' ")
        conn.commit()
